...
Both `event_msg:user_message.images` and `item_completed.UserMessage.content` can hold embedded UI images. A completed UI projection replaces the pending model bytes only with an explicit matching turn ID, normalized request text and ordered image count; it never creates another prompt record. Preserve the ordered group byte hashes of all corroborated representations for identity resolution before publication, even without local attachments. The private management API accepts up to eight fingerprints per record (UI/model/original bytes and pixel fingerprints). Two active targets remain a conflict; neither a shared prompt nor perceptual similarity permits an automatic merge. Regression tests must cover importing model bytes first and UI bytes later, the reverse order, missing attachments, multi-image groups and mismatched turns.
...
The import plan reports prompts for new cards separately from additional prompts on published cards, and numbers duplicate/variant records from one. The startup explanation and `--help` distinguish JSONL-embedded UI/model images from verified local attachments, define records/cards/source/thumbnail counts, and explain duplicate Prompt and tag behavior. The original-recovery report partitions recovered image occurrences by the same eligibility decision used to plan writes: replacement, already current, larger published image, incomplete attachment group, new card, or default preservation. It then counts unique replacement cards, explicitly accounting for repeated records and URL-only corrections. With `--overwrite-images`, it states that replacement is enabled; `--dry-run` always states that no writes will occur. Image/URL replacement runs only with explicit `--overwrite-images`, before normal metadata batches. These replacements use batches of 10: each card moves two detail objects, and HF write latency must leave time for shared-index publication and rollback within the function deadline. Ordinary metadata batches retain their existing size. A replacement POST is sent once. If its response is lost, the importer polls read-only identity resolution until every requested image hash and target URL are confirmed or the deadline expires; it does not replay a potentially running mutation. An unresolved conflict stops the import. A complete readback is reported as confirmed publication rather than zero changes. The replacement endpoint returns `changedSlugs` alongside its existing aggregate count, allowing CLI output to distinguish prompt-only, image/URL-only and combined changes from acknowledged results. The items endpoint returns `promptChangedHashes` from each committed candidate versus its fresh public prompt revision; a metadata-only or duplicate upsert is not a prompt update. Metadata batch image replacements are always zero; tags and original/thumbnail asset files have separate counts.
...
运行 `npm run import:style-prompts -- --help` 可以查看完整用法，不需要 token、会话文件或网络。正式导入的开头也会显示图片来源、去重规则、当前模式、标签行为和结果计数的简要说明。第一次运行建议先加 `--dry-run`：它需要管理 token 来查询线上身份，但不会上传文件或改写数据。

“找到原图”和“需要替换卡片”使用不同的计数单位。前者统计每条消息中成功找回的单张图片，后者统计去重后确实需要变更的已有卡片。例如，找回 271 张图片，其中 23 张线上已经是同一原图，则只需替换剩余 248 张。若同一卡片在会话中出现多次，或一张卡片包含多张图片，还需按日志中的归并说明计算，不能直接相减。新卡片使用的原图、URL 校正以及不可用附件会单独列出。

批次响应中的“重复”也可能出现在本轮刚新增的记录上：第一次请求已保存成功，但响应丢失，重试就会命中已保存的内容。普通导入的最终汇总会复用发布后的回读，与本轮开始时的卡片和 Prompt 比较，确认新增卡片、新 Prompt 与重复记录的数量。批次响应仅说明该次请求的结果；判断整轮导入改了多少数据，应看最终回读汇总。

覆盖图片与覆盖标签互不包含。`--overwrite-images` 只决定是否修复已有图片及 URL；`--tag` 追加标签，只有同时指定 `--overwrite-tag` 才会替换标签集合。没有 `--tag` 时不处理标签。图片和 Prompt 重复的记录仍会参与显式指定的标签操作。
