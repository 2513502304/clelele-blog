...
The collection dialog has one scrollable form body with a fixed header and footer; textareas and tag suggestions scroll within it. Focusing tags brings the suggestion area into view. The page scroll root stays locked until the dialog closes. Selected files occupy one compact, three-layer fan stack regardless of count. Clicking it opens a local-only image viewer with zoom and navigation; blob URLs are revoked when selection changes or the dialog unmounts. Previewing files does not upload or sign assets.
