...
The session importer first extracts embedded images, using a nonempty UI `event_msg:user_message.images` record when present in the observed turn order, and then attempts recovery only for validated structured attachments. The ten-original audit in `docs/research/2026-09-20-codex-ten-originals-verification.md` confirms five VS Code UI images preserve exact original bytes, including oversized inputs. Desktop immediate and queued inputs can contain resized bytes; attachment icons do not identify this. The current recovery branch does not consume `event_msg:user_message.local_images` alone. New Codex desktop sessions can contain a resized/re-encoded `response_item.input_image` while the UI `item_completed.UserMessage.local_image` retains the original file path. The importer pairs them only within the same `turn_id`, with matching ordered attachment-wrapper paths and image counts. It validates the format, aspect ratio and a small decoded comparison to reject obviously replaced local files; this comparison is a path-integrity check, not a cross-card deduplication rule. Missing/invalid originals fall back to embedded bytes with a warning. Normal reimports first resolve embedded byte identities and preserve an already-published card. Only genuinely new image groups recover attachments by default; existing cards may still receive new prompt variants or requested tags. No local paths are published. Import while temporary clipboard files remain available; an archived session alone cannot reconstruct original compressed bytes.
