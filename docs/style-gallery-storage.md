...
Original recovery accepts both `user_message.local_images` and `item_completed.UserMessage.local_image`. The former must corroborate the pending model input's request text and ordered renderer-wrapper paths within the same task; the latter also requires the matching turn ID. Replacing a model projection with its UI images retains validated attachment state. Arbitrary paths mentioned in prompt text are never opened. Before using an attachment, the importer checks format, aspect ratio and a small decoded comparison to reject an obviously replaced file. This is a path-integrity check, not permission to merge visually similar cards.
