...
- Selected cards need both a clear outline and a subtle glow. Dragging the dock uses a dedicated handle (with arrow-key support), not its buttons. Pointer capture keeps dragging outside the handle; resize/content changes clamp a moved dock back into the viewport. Collapse remains independent from position.
