...
- Selecting images must occupy one bounded fan stack, not a growing grid. Recheck with the maximum supported selection, long prompts and a narrow/short viewport.
...
- Nested image previews preserve the parent draft and page lock. Closing the viewer must not close the editor. Revoke locally created blob URLs, never remote URLs. Previews must not upload data.
