...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (body.action === 'cleanup') {
      ...
      const storedItems = await mapWithConcurrency(catalog.items, 8, (item) =>
        getStoredStyleGalleryItem(item.slug, { fresh: true }),
      );
      ...
      await mapWithConcurrency(removableKeys, 8, deleteStyleGalleryObject);
      return Response.json({ deleted: removableKeys.length, retained: keys.length - removableKeys.length });
    }

    const uploads = await mapWithConcurrency(keys, 8, async (key) => {
