  ...
  return (
    <>
      <button
        ...
        className="flex h-9 shrink-0 items-center gap-2 rounded-lg border border-border bg-background px-3 text-sm hover:border-primary/40 hover:text-primary"
      >
              ...
              >
                {([0, 1] as const).map((side) => (
                  <label key={side} className="text-sm">
                    {zh ? `卡片 ${side + 1} 的 hash` : `Card ${side + 1} hash`}
                    <input
                      required
                      ...
                      onChange={(event) => {
                        setHashes((current) =>
                          side === 0 ? [event.target.value.trim(), current[1]] : [current[0], event.target.value.trim()],
                        );
                      }}
                      placeholder="12–64 位 hash"
                ...
                <button
                  type="submit"
                  className="self-end rounded-lg border border-border px-4 py-3 text-sm hover:border-primary"
                >
