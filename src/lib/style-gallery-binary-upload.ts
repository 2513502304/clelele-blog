 ...
 */
async function handleDirectUpload(request: Request, url: URL, kind: 'source' | 'example'): Promise<Response> {
 ...
 */
async function handleCompleteUpload(rawBody: unknown, kind: 'source' | 'example'): Promise<Response> {
  ...
  const finalKey =
    kind === 'source'
      ? `source/${body.imageHash.slice(0, 12)}.${body.extension}`
      : getStyleGalleryExampleKey(body.imageHash, body.extension);
  if (await headStyleGalleryObject(finalKey)) {
...
/** Shared transport; callers must authenticate before reading any bytes or creating upload parts. */
export async function handleStyleGalleryBinaryUpload(
  request: Request,
  url: URL,
  kind: 'source' | 'example',
): Promise<Response> {
  if (url.searchParams.get('action') === 'direct') return handleDirectUpload(request, url, kind);
  if (url.searchParams.get('action') === 'chunk') return handleChunkUpload(request, url);
  const rawBody = await request.json();
  if (rawBody?.action === 'complete') return handleCompleteUpload(rawBody, kind);
