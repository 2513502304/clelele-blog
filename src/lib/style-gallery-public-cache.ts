...
export const STYLE_GALLERY_VERCEL_CACHE_CONTROL = 'public, s-maxage=21600, stale-while-revalidate=604800';
...
/**
 * 浏览器每次仍向站点确认新鲜度，Vercel 边缘节点则复用 SSR 结果 6 小时；写操作通过 tag 精准清除。
...
/**
 * Gallery 持久化成功后才清除 CDN：列表 tag 覆盖预览、矩阵、Sub-gallery 和搜索索引，item tag
 * 只清除发生变化的详情与 prompt 响应。清除失败不能回滚已经提交到 HF 的事实，因此仅记录错误。
 */
export async function invalidateStyleGalleryPublicCache(slugs: readonly string[] = []): Promise<void> {
  ...
  try {
    await dangerouslyDeleteByTag([...new Set(tags)]);
  } catch (error) {
