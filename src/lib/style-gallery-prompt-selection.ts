...
const selectedPromptBySlug = new Map<string, StyleGalleryPromptSelectedDetail>();
...
export function getSelectedStyleGalleryPrompt(slug: string): string | undefined {
  return selectedPromptBySlug.get(slug)?.prompt;
}
...
export function selectStyleGalleryPrompt(detail: StyleGalleryPromptSelectedDetail): void {
  selectedPromptBySlug.set(detail.slug, detail);
  window.dispatchEvent(new CustomEvent<StyleGalleryPromptSelectedDetail>(STYLE_GALLERY_PROMPT_SELECTED_EVENT, { detail }));
}

/** Distinguishes no selection from a selected variant without an original user prompt. */
export function getSelectedStyleGalleryPromptDetail(slug: string): StyleGalleryPromptSelectedDetail | undefined {
