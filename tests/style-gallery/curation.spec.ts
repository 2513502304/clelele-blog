...
for (const mobile of [false, true]) {
  for (const path of ['', '/index']) {
    test(`manual collection form keeps drafts and fits ${mobile ? 'mobile' : 'desktop'} ${path || 'preview'}`, async ({
      ...
      await dialog.getByLabel('模型反推 Prompt').fill('Collected test prompt');
      page.once('dialog', (confirmation) => confirmation.dismiss());
      await dialog.getByRole('button', { name: '取消', exact: true }).click();
      await expect(dialog.getByLabel('模型反推 Prompt')).toHaveValue('Collected test prompt');
...
test('detail prompt edit preserves a rejected draft and publishes successful text to copy/display', async ({ page }) => {
    ...
    if (route.request().method() === 'PATCH' && route.request().url().includes('/api/style-gallery/prompts/')) {
      ...
      return route.fulfill({
        json: {
          ...
          prompts: [{ id: 'b'.repeat(64), prompt: body.prompt, model: 'Collected', importedAt: new Date().toISOString() }],
        },
      });
    }
    return route.continue();
  ...
  await page.getByRole('button', { name: '编辑 Prompt', exact: true }).click();
